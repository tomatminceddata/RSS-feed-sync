...
Because the pages are unreachable, both local scripts fall back to the article body carried in the feed's `<description>` (`entry.summary`). `rss_sync.py` reads it straight from the live feed it already iterates; `rss_pull_and_sync.py` works from a queue of URLs, so it parses the matching feed once and looks each failed URL up in it — a URL that has aged out of the ~20-entry feed window has no fallback and stays pending. Either way, the fallback works well for the roughly half of posts that ship full-text in the feed. The other half arrive as **stubs** — a one- or two-sentence teaser — with the real article living only on the walled page.
